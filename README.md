...
- Rain interrupts the chase specifically, but the match can still finish? "Revise target", in the same menu as "End innings"/"Abandon match", only shown once the second innings is under way. Sets a new target and overs limit — whatever both sides have actually agreed on, not a calculated par score; this app doesn't attempt full DLS math. Everything downstream follows the revised numbers from that point on: the live target/required-rate display, when the innings actually ends, the final result text, and (in a tournament) NRR, which credits the chasing side with the revised overs rather than the original ones if they end up all out. Can be revised again later if play's interrupted a second time — the latest revision is always what counts.
...
- All of the above — creating/joining/renaming clubs, member and co-owner management, federation affiliation — lives on the **Teams** screen, under its own **Clubs** and **Federations** tabs alongside the roster-picking **Teams** tab. Used to live as a separate screen reached only via Account; folded in as tabs instead so managing a club doesn't mean navigating away from where you're already picking rosters. Account's "Manage Clubs & Federations" button still works as a shortcut — it jumps straight to the Clubs tab.
