...
- Automatic strike rotation, bowler tracking, and over completion. The over-by-over ball strip shows each delivery's over.ball number (e.g. `2.1`, `2.2`); a wide or no-ball doesn't consume a legal-ball slot, so it's labelled with the same number as whichever legal delivery eventually completes it.
...
**Teams & clubs**
- Save team rosters once, reuse them across matches. Set a captain and wicketkeeper per team; they're marked on the scorecard. Give a team a jersey colour and it shows as a small swatch next to its name in the team list.
