...
export function FollowTournamentScreen({
  ...
  // Most recently played first -- reported live: results showing in no discernible order.
  // Mirrors scheduledFixtures' own date comparator below, just reversed (recent-first for history,
  // soonest-first for what's still ahead). An undated completed fixture (rare, but a fixture can in
  // principle carry a result with no date ever set) sorts after every dated one rather than
  // jumbling the dated ones around it.
  const completedFixtures = [...fixtures].filter(f => f.result).sort((a, b) => {
