...
export const HELP_SECTIONS = [{
  ...
  entries: [{
    ...
    a: "Yes \u2014 the Tournament Stats tab (batting/bowling leaderboard) has an \u201cExport CSV\u201d button, exporting whichever of the two is currently showing. Opens straight in Excel, Google Sheets, or Numbers."
  }, {
...
export function AboutScreen({
  ...
  }, "What's stored on this device vs. synced, and who can see it \u2014 tap to read."), privacyOpen && ["Scoring on this device alone needs nothing from you \u2014 no account, and nothing leaves your phone.", "Signing in to sync across devices or share a live-view link stores your email and whatever team and player details you enter (names, roles, batting/bowling hand, notes) in Firestore, Google's cloud database, under this app's own project. A live-view link is read-only and works for anyone with the link or code; there's no login on the viewing side.", "Any match currently in progress also appears in the Home tab's Matches section and app-wide search \u2014 visible to anyone using the app, not just people you've sent a link to \u2014 and stays findable there for a few days after it ends, not just while it's live. A public tournament works the same way: its standings and fixtures show up in the Home tab's Tournaments section and refresh themselves automatically as results come in, with nothing extra to turn on \u2014 there's no separate \u201cshare\u201d step. Whether a match or tournament is private or public is its own explicit choice, made when it's created (defaulting to Public) \u2014 a fixture started from within a tournament inherits that tournament's own choice instead of making its own. A private one stays out of these feeds entirely, live or after completion; a share or view link you generate yourself still works exactly the same either way. A match scored fully offline, with no account and no match code, never appears here at all.", "A finished match's recap (Result screen) is built entirely on your device from that match's own data \u2014 nothing sent anywhere. Tapping \u201cPolish with AI\u201d on it is the one exception: that recap text, which can include real players' names, is sent to a third-party AI provider (Google Gemini) to rephrase it. It's optional, available only to whoever has that match's own result screen open (never offered on a public live-follow link), and skipping it leaves the plain recap exactly as useful.", "There are no analytics, no trackers, and no ads in this app \u2014 nothing here is sold or shared with advertisers."].map((para, idx) => /*#__PURE__*/React.createElement("div", {
    key: idx,
    style: {
      fontFamily: "'Inter'",
      fontSize: 12,
