...
export const HELP_SECTIONS = [{
  ...
  entries: [{
    ...
    q: "What's the \"Match recap\" on the result screen, and what does \"Polish with AI\" actually send anywhere?",
    a: "The recap text itself is built entirely on your device from the match's own result, top scorer/bowler, and milestones — no network call, works offline, shown automatically once a match finishes. \"Polish with AI\" is a separate, optional tap: it sends that recap text (which can include real players' names) to a third-party AI provider to rephrase it, and isn't offered on the public live-follow page — only to whoever has the match's own result screen open. Skip it and the plain recap stays exactly as useful; nothing about scoring, sharing, or the scorecard depends on it."
  }, {
  ...
  entries: [{
    ...
    q: "What's the difference between a club's Player Pool and a team's roster?",
    a: "The Player Pool (Teams screen \u2192 a club \u2192 Add from club pool) is a club-wide list of everyone available to draw onto any of that club's teams \u2014 add someone once, reuse them on as many rosters as you like. A team's roster is just that one team's actual line-up. Typing a brand-new name straight into a roster adds them to the pool too, by default (there's an opt-out toggle right there when adding), so most rosters stay in sync automatically. A roster built before the club had a pool, or added with that toggle off, can end up with names the pool doesn't know about \u2014 a small checkmark next to a club roster player means they're already in the pool; a compact \u201cPool\u201d button appears instead when they're not, adding them without leaving the screen."
  }, {
...
export function AboutScreen({
  ...
  // Same reasoning as privacyOpen just above.
  ...
  // Ends the card on its one call to action instead of trailing off into privacy copy -- placed
  // after Data & privacy in render order below, even though it's defined first here.
  const supportBlock = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: sectionLabelRowStyle
  ...
  const privacyBlock = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    type: "button",
  ...
  }, "What's stored on this device vs. synced, and who can see it \u2014 tap to read."), privacyOpen && ["Scoring on this device alone needs nothing from you \u2014 no account, and nothing leaves your phone.", "Signing in to sync across devices or share a live-view link stores your email and whatever team and player details you enter (names, roles, batting/bowling hand, notes) in Firestore, Google's cloud database, under this app's own project. A live-view link is read-only and works for anyone with the link or code; there's no login on the viewing side.", "Any match currently in progress also appears in the Home screen's Live now feed and app-wide search \u2014 visible to anyone using the app, not just people you've sent a link to \u2014 and stays findable there for a few days after it ends, not just while it's live. A public tournament works the same way: its standings and fixtures show up in the Home screen's Live tournaments strip and refresh themselves automatically as results come in, with nothing extra to turn on \u2014 there's no separate \u201cshare\u201d step. Whether a match or tournament is private or public isn't a separate choice \u2014 it follows its Organizer: Personal is always private, a club or federation is always public (a fixture started from within a tournament inherits that tournament's own Organizer). A private one stays out of these feeds entirely, live or after completion; a share or view link you generate yourself still works exactly the same either way. A match scored fully offline, with no account and no match code, never appears here at all.", "Publishing a player to the cross-club directory makes their name, email, and those same details visible to other clubs searching for them, until you make them private again. Turning on \u201cDiscoverable for invites\u201d in Account does the same for your own account \u2014 your name, email, and photo become findable by name to any club or federation owner sending an invite, until you turn it off.", "A finished match's recap (Result screen) is built entirely on your device from that match's own data \u2014 nothing sent anywhere. Tapping \u201cPolish with AI\u201d on it is the one exception: that recap text, which can include real players' names, is sent to a third-party AI provider (Google Gemini or Groq) to rephrase it. It's optional, available only to whoever has that match's own result screen open (never offered on a public live-follow link), and skipping it leaves the plain recap exactly as useful.", "There are no analytics, no trackers, and no ads in this app \u2014 nothing here is sold or shared with advertisers."].map((para, idx) => /*#__PURE__*/React.createElement("div", {
    key: idx,
    style: {
      fontFamily: "'Inter'",
      fontSize: 12,
      color: COLORS.inkSoft,
      lineHeight: 1.7,
      marginBottom: idx < 5 ? 10 : 0
    }
  }, para)));
  // Same collapsed-by-default pattern as privacyBlock just above, same reasoning -- policy text
  // nobody reads start to finish, placed right next to Data & privacy since together they're
  // "the legal stuff," ahead of Support (which deliberately stays the card's last, most inviting
  // word -- see its own comment).
  const termsBlock = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    ...
    style: sectionLabelTextStyle
  }, "Terms of service")), /*#__PURE__*/React.createElement(ChevronDown, {
    size: 15,
    style: {
      color: COLORS.inkSoft,
  ...
  })), !termsOpen && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Inter'",
      fontSize: 12,
      color: COLORS.inkSoft,
  ...
  }, "The short version of what using this app means — tap to read."), termsOpen && ["Club Scorer is free, provided as-is with no warranty of any kind. It's built and maintained by one person as a side project for club cricket — treat any score, standing, or DLS number it produces as a convenience, not an official record, and keep your own scorebook for anything that actually matters (a league table, a trophy, a dispute).", "You're responsible for what you enter — player names, photos, notes, club and federation details — and for having the right to share it, especially anything made visible to others (a published player, a live share link, a public club or federation directory entry, or a match recap sent to a third-party AI provider via “Polish with AI” — see “Data & privacy” above). Don't use the app to harass, impersonate, or publish someone else's information without their OK.", "The service can change, have downtime, or be discontinued at any time, with or without notice — it runs on free-tier infrastructure on a best-effort basis, not a guaranteed uptime commitment. Nothing here is sold, and there's no paywall or subscription (see “Support” below) that creates any entitlement beyond what's written here.", "Because it's open source under the MIT License (see above), you're always free to copy the code and run your own instance if this one ever changes in a way you don't want to depend on — see “Running your own copy” in the README for exactly how.", "These terms may change as the app does; continuing to use it after an update means accepting the current version. See “Data & privacy” above for how your information is actually handled."].map((para, idx) => /*#__PURE__*/React.createElement("div", {
    key: idx,
    style: {
      fontFamily: "'Inter'",
      fontSize: 12,
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    style: dividerStyle
  }), privacyBlock, /*#__PURE__*/React.createElement("div", {
    style: dividerStyle
  }), termsBlock, /*#__PURE__*/React.createElement("div", {
    style: dividerStyle
  }), supportBlock)));
}
