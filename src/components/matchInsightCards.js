...
export function PlayerOfMatchCard({
  ...
  async function pick(name) {
    ...
    setMatch(updated);
    setPicking(false);
    let result = await saveMatch(updated);
    if (result.conflict && result.remoteMatch) {
      // Something else saved this match in between (the Best Fielder card right below picked at
      // the same moment, most likely, or a device still live-scoring) -- retry against the
      // server's actual current document with just this one field applied, rather than resending
      // our own now-stale copy and silently erasing whatever that other save just committed.
      // Without this, the conflict used to be dropped entirely: no error, no retry, and this
      // card's local writeSeq was left permanently behind the server's real one, so every
      // following pick kept losing the same race forever -- which read as "the award can't be
      // changed" once it happened once.
      const retried = {
        ...result.remoteMatch,
        playerOfMatch: name
      };
      result = await saveMatch(retried);
      if (result.ok) {
        setMatch(cur => cur ? {
          ...retried,
...
export function BestFielderCard({
  ...
  async function pick(name) {
    ...
    let result = await saveMatch(updated);
    if (result.conflict && result.remoteMatch) {
      // Something else saved this match in between (the Player of the Match card right above
      // picked at the same moment, most likely, or a device still live-scoring) -- retry against
      // the server's actual current document with just this one field applied, rather than
      // resending our own now-stale copy and silently erasing whatever that other save just
      // committed. Without this, the conflict used to be dropped entirely: no error, no retry,
      // and this card's local writeSeq was left permanently behind the server's real one, so
      // every following pick kept losing the same race forever -- which read as "the award can't
      // be changed" once it happened once.
      const retried = {
        ...result.remoteMatch,
        bestFielder: name
      };
