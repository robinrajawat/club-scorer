...
export function MatchScreen({
  ...
  // Measures the fixed bottom scoring pad's REAL rendered height, rather than guessing it with a
  // hardcoded pixel number (see the paddingBottom this feeds, below). The old hardcoded guess
  // (40 collapsed / 118 expanded) was tuned once and never revisited as the pad grew new
  // conditional rows -- the Big Hit/Maximum Hit button row chief among them -- so on a match with
  // either configured, or on a device with a tall safe-area-inset-bottom (a notched/home-indicator
  // iPhone), the pad's real height regularly exceeds the reserved space. Any content flowing close
  // to the bottom of the page then renders UNDERNEATH the fixed pad instead of above it -- clipped,
  // partially hidden, or with just a sliver peeking out past the pad's rounded top corner. That's
  // the leading theory for the long-reported "This Over" strip rendering glitches (content
  // sometimes hidden/misaligned near the bottom of the screen) -- a real per-device/per-match
  // layout bug, not a data or reconciliation issue in OversStrip itself (which was investigated at
  // length and never showed anything wrong with its actual DOM output).
  const bottomPadRef = useRef(null);
  const [bottomPadHeight, setBottomPadHeight] = useState(0);
  ...
  // The one-line ball commentary has no self-clearing timer (unlike celebration/milestoneToast,
  // both of which dismiss themselves) -- it's meant to persist until the next ball. That's right
  // mid-innings, but MatchScreen itself never unmounts across the innings break (it just renders
  // SecondInningsSetup in place of the normal scoring UI while awaitingSecondInningsSetup is true,
  // then reverts to this same component instance), so without this the LAST ball of the first
  // innings kept showing here as soon as the second innings' scoring screen appeared, describing a
  // delivery from a different innings entirely.
  useEffect(() => {
  ...
  // `!inning.nonStrikerName` alongside the long-standing `!inning.strikerName` check -- every real
  // dismissal still always vacates the striker's slot (unchanged), so that half is a no-op for
  // them; it only starts mattering for retireBatsman's non-striker branch, which is the one path
  // that can leave the NON-striker's slot empty instead (see its own comment).
  const needsNewBatsman = inning && (!inning.strikerName || !inning.nonStrikerName) && inning.wickets < maxWicketsFor(match, inning) && !inning.complete;
  ...
  // Keeps bottomPadHeight in sync with the fixed pad's actual rendered size -- ResizeObserver
  // picks up every reason that size can change (padCollapsed, the Big Hit/Maximum Hit row
  // appearing or disappearing, a bowler-name line wrapping, safe-area-inset-bottom differing by
  // device) automatically, without needing any of those as explicit effect dependencies. Reads
  // getBoundingClientRect() rather than the observer entry's own contentRect, which excludes the
  // pad's own padding -- exactly the part of its height this needs to account for.
  useEffect(() => {
    const el = bottomPadRef.current;
    if (!el) {
      setBottomPadHeight(0);
      return;
    }
    const measure = () => setBottomPadHeight(el.getBoundingClientRect().height);
    measure();
    const observer = new ResizeObserver(measure);
    observer.observe(el);
    return () => observer.disconnect();
  }, [needsNewBatsman, needsNewBowler]);
  ...
  function confirmNewBatsman() {
    ...
    if (pendingWicket) {
      ...
      // Same fix as confirmNewBowler's own -- a committed action with no preceding pushHistory()
      // has no checkpoint of its own for Undo to step back to, so "Undo last ball" right after
      // picking the wrong replacement here (e.g. after a retirement) silently reverted the
      // PREVIOUS delivery instead. This branch only runs when there's no pendingWicket (the
      // pendingWicket branch above already pushes its own history, since that commit IS the wicket
      // delivery itself) -- this one is purely "assign a name to the open slot," same shape as
      // confirmNewBowler. The open slot is whichever end retireBatsman actually vacated -- almost
      // always the striker's (every real dismissal, and most retirements), but a retireBatsman
      // call for the non-striker (see its own comment) leaves nonStrikerName empty instead, and
      // the new batsman needs to fill that same end, not bump the survivor off strike.
      pushHistory();
      const openSlot = !inning.strikerName ? "strikerName" : "nonStrikerName";
      const updated = {
        ...inning,
        [openSlot]: newBatsmanName.trim()
      };
      ensureBatsman(updated, updated[openSlot]);
      commit(updated);
    }
  ...
  function confirmNewBowler() {
    ...
    // Undo genuinely reverts it). Without it, "Undo last ball" right after picking the wrong bowler
    // for a new over had no checkpoint to step back to for THIS pick -- it silently reverted the
    // PREVIOUS delivery instead (the ball that ended the last over), which happens every single
    // over of every match, not some rare edge case.
    pushHistory();
    const updated = {
      ...inning,
      bowlerName: name
  ...
  // directly, same pattern as swapStrike above. `name` (defaulting to whoever's currently on
  // strike, for the manual Retire popup's existing call sites) is looked up against BOTH ends —
  // vacating whichever one actually matches, rather than always assuming the striker's slot the
  // way every real dismissal in the ball reducer does (see dismissedName there). This is what lets
  // the retirement-cap prompt (needsCapRetirement below) retire the non-striker directly instead
  // of forcing a Swap Strike first: the survivor's own end is never touched either way, so there's
  // nothing to "swap back" once the next batsman is in — see needsNewBatsman/confirmNewBatsman,
  // which were generalized the same way to fill whichever end this leaves empty.
  ...
  function retireBatsman(kind, name = inning.strikerName) {
    if (!name || (name !== inning.strikerName && name !== inning.nonStrikerName)) return;
    pushHistory();
    const isOut = kind === "out";
    if (!isOut) setJustRetiredName(name);
    const wasStriker = name === inning.strikerName;
    const survivor = wasStriker ? inning.nonStrikerName : inning.strikerName;
    ...
    const updated = {
      ...
      strikerName: wasStriker ? "" : inning.strikerName,
      nonStrikerName: wasStriker ? inning.nonStrikerName : "",
      ...
      partnerships: survivor ? [...(inning.partnerships || []), {
        batter1: name,
        batter2: survivor,
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: {
      paddingBottom: bottomPadHeight ? bottomPadHeight + 8 : 24,
      maxWidth: 560,
      margin: "0 auto"
    }
  ...
  }, `Reached ${inning.retirementRuns} runs — this tournament's rules require retiring at that point (not out, not a dismissal). They can return later once the rest of the batting order has had a turn.`), /*#__PURE__*/React.createElement(Btn, {
    onClick: () => retireBatsman("cap", capRetireName),
    style: {
      width: "100%",
      marginBottom: 10
    }
