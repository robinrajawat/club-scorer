...
export function TeamEditScreen({
  ...
  // own dedup checks already use, since neither side carries a stable link back to the other.
  function isInPool(p) {
    return clubPool.some(pp => pp.name.trim().toLowerCase() === p.name.trim().toLowerCase());
  }
  // Retrofits a roster row that predates the pool (or had alsoAddToPool off, or was added before
  // this club had a pool at all) into it, without leaving this screen. Carries over whatever role/
  // hand/email this row already has, same fields addPoolPlayers itself accepts.
  function addToPoolFromRoster(p) {
    if (!onAddPoolPlayers) return;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    type: "button",
    onClick: () => addToPoolFromRoster(p),
    className: "cs-btn",
    "aria-label": `Add ${p.name} to the club player pool`,
    title: "Not in the club player pool yet — tap to add.",
    style: {
      flexShrink: 0,
      display: "flex",
      alignItems: "center",
