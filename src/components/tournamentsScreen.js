...
import { AlertTriangle, ArrowLeftRight, Bell, ChevronRight, Info, Pencil, Trophy } from "./icons.js";
...
// else in this suite) backs the create-series dialog only -- create-tournament is an inline card,
// not a modal. Covered by
...
export function TournamentsScreen({
  ...
  // See isPrivate's own comment further down -- the create-tournament form's explicit Visibility
  // choice, independent of Organizer.
  ...
  // Which of the two "New …" flows the floating "+" opens -- see its own comment lower down.
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Tournaments & series \u2014 group matches together for a running points table, or track a run of head-to-head games instead. Tag a match with a tournament from its setup screen."), tournaments.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 14
    }
  }, /*#__PURE__*/React.createElement(TextField, {
  ...
  }, seriesBusy ? "Creating\u2026" : "Create series")), creating && /*#__PURE__*/React.createElement("div", {
    style: {
      background: COLORS.surface,
      borderRadius: 16,
      padding: 16,
  ...
  }, t.kind === "series" ? `${(t.fixtures || []).length} match${(t.fixtures || []).length === 1 ? "" : "es"}` : `${t.teams.length} teams`, tournamentDateRangeLabel(t) && ` · ${tournamentDateRangeLabel(t)}`, t._clubId && ` · ${(clubs.find(c => c.id === t._clubId) || {}).name || "a club"}`, t._federationId && ` · ${(myFederations.find(f => f.id === t._federationId) || {}).name || "a federation"}`)), /*#__PURE__*/React.createElement(ChevronRight, {
    size: 18,
    style: {
      color: COLORS.inkSoft,
      flexShrink: 0
    ...
    // place every other screen starts a create flow from. Folding it in here means one obvious
    // entry point for both, at the cost of one extra tap for the (more common) Tournament case.
    onClick: () => setShowCreateMenu(true),
    label: "New"
  }), showCreateMenu && /*#__PURE__*/React.createElement(Modal, {
    ...
    style: { fontFamily: "'Inter'", fontWeight: 700, fontSize: 14, color: COLORS.ink }
  }, "Tournament"), /*#__PURE__*/React.createElement("div", {
    style: { fontFamily: "'Inter'", fontSize: 12, color: COLORS.inkSoft, marginTop: 2 }
  }, "A group of teams, a running points table."))), /*#__PURE__*/React.createElement("button", {
    type: "button",
    onClick: () => {
      setShowCreateMenu(false);
      openCreateSeries();
    ...
    className: "cs-btn",
    "aria-label": "New head-to-head series",
    style: {
      display: "flex",
      alignItems: "center",
      gap: 12,
    ...
    style: { fontFamily: "'Inter'", fontWeight: 700, fontSize: 14, color: COLORS.ink }
  }, "Head-to-head series"), /*#__PURE__*/React.createElement("div", {
    style: { fontFamily: "'Inter'", fontSize: 12, color: COLORS.inkSoft, marginTop: 2 }
  }, "A running score between two teams across several matches.")))));
}
